...
ring_log* ring_log::_ins2 = NULL;
pthread_once_t ring_log::_once2 = PTHREAD_ONCE_INIT;
...
void ring_log::persist()
{
    while (true)
    {
        ...
        int year = _tm.year, mon = _tm.mon, day = _tm.day, hour = _tm.hour;
        pthread_mutex_unlock(&_mutex);

        //decision which file to write
        if (!decis_file(year, mon, day, hour))
            continue;
        //write
...
void ring_log::try_append(const char* lvl, const char* format, ...)
{
    ...
    //int prev_len = snprintf(log_line, LOG_LEN_LIMIT, "%s[%d-%02d-%02d %02d:%02d:%02d.%03d]", lvl, _tm.year, _tm.mon, _tm.day, _tm.hour, _tm.min, _tm.sec, ms);
        int prev_len = snprintf(log_line, LOG_LEN_LIMIT, "%s.%03d\t%s\t", _tm.utc_fmt, ms, lvl);
...
bool ring_log::decis_file(int year, int mon, int day, int hour)
{
    ...
    const char* Levelstr = getstringbylevel(_level);
    if (!_fp)
    {
        _year = year, _mon = mon, _day = day, _hour = hour;
        char log_path[1024] = {};
        sprintf(log_path, "%s/%s.%s.%d%02d%02d%02d.%u.log", _log_dir, _prog_name, Levelstr, _year, _mon, _day, _hour, _pid);
    ...
    else if (ftell(_fp) >= LOG_USE_LIMIT)
    {
        ...
        {
            sprintf(old_path, "%s/%s.%s.%d%02d%02d%02d.%u.log.%d", _log_dir, _prog_name, Levelstr, _year, _mon, _day, _hour, _pid, i);
            sprintf(new_path, "%s/%s.%s.%d%02d%02d%02d.%u.log.%d", _log_dir, _prog_name, Levelstr, _year, _mon, _day, _hour, _pid, i + 1);
            rename(old_path, new_path);
        }
        //mv xxx.log xxx.log.1
        sprintf(old_path, "%s/%s.%s.%d%02d%02d%02d.%u.log", _log_dir, _prog_name, Levelstr, _year, _mon, _day, _hour, _pid);
        sprintf(new_path, "%s/%s.%s.%d%02d%02d%02d.%u.log.1", _log_dir, _prog_name, Levelstr, _year, _mon, _day, _hour, _pid);
        rename(old_path, new_path);
...
{
    ring_log::ins(1)->persist();
    return NULL;
}

void* be_thdo2(void* args)
{
    ring_log::ins(2)->persist();
    return NULL;
}
