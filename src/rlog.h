...
public:
    ...
    static ring_log* ins(int log_type = 1)
    {
        if (log_type == 1) {
            ...
            pthread_once(&_once2, ring_log::init2);
            return _ins2;
        }
    }
    ...
    static void init2()
    {
        while (!_ins2) _ins2 = new ring_log();
        atexit(destroy2);
    }
...
private:
    ...
    bool decis_file(int year, int mon, int day, int hour);
    ...
    FILE* _fp;
    pid_t _pid;
    int _year, _mon, _day, _hour, _log_cnt;
    char _prog_name[128];
    char _log_dir[512];
    ...
    static ring_log* _ins2;
    static pthread_once_t _once;
    static pthread_once_t _once2;
};

void* be_thdo(void* args);
void* be_thdo2(void* args);
...
#define LOG_INIT(log_dir, prog_name, level) \
    do \
    { \
        ring_log::ins(1)->init_path(log_dir, prog_name, level); \
        pthread_t tid; \
        pthread_create(&tid, NULL, be_thdo, NULL); \
        pthread_detach(tid); \
    } while (0)

#define SEVICE_LOG_INIT(log_dir, prog_name, level) \
    do \
    { \
        ring_log::ins(2)->init_path(log_dir, prog_name, level); \
        pthread_t tid; \
        pthread_create(&tid, NULL, be_thdo2, NULL); \
        pthread_detach(tid); \
    } while (0)

//format: [LEVEL][yy-mm-dd h:m:s.ms][tid]file_name:line_no(func_name):content
#define SERVICE_LOG_TRACE(fmt, args...) \
    do \
    { \
        if (ring_log::ins(2)->get_level() >= TRACE) \
        { \
            ring_log::ins(2)->try_append("[TRACE]", fmt "\n", \
                     ##args); \
        } \
    } while (0)


#define RLOG_TRACE(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= TRACE) \
        { \
            ring_log::ins()->try_append("[TRACE]", fmt "\n", \
                     ##args); \
        } \
    } while (0)

#define RLOG_DEBUG(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= DEBUG2) \
        { \
            ring_log::ins()->try_append("[DEBUG]", fmt "\n", \
                    ##args); \
        } \
    } while (0)

#define SERVICE_LOG_DEBUG(fmt, args...) \
    do \
    { \
        if (ring_log::ins(2)->get_level() >= DEBUG2) \
        { \
            ring_log::ins(2)->try_append("[DEBUG]", fmt "\n", \
                    ##args); \
        } \
    } while (0)
...
#define RLOG_INFO(fmt, args...) \
    ...
    { \
        ...
        { \
            ring_log::ins()->try_append("[INFO]", fmt "\n", \
                    ##args); \
        } \
    } while (0)

#define SERVICE_LOG_INFO(fmt, args...) \
    do \
    { \
        if (ring_log::ins(2)->get_level() >= INFO) \
        { \
            ring_log::ins(2)->try_append("[INFO]", fmt "\n", \
                    ##args); \
        } \
    } while (0)

#define RLOG_NORMAL(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= INFO) \
        { \
...
#define SERVICE_LOG_NORMAL(fmt, args...) \
    do \
    { \
        if (ring_log::ins(2)->get_level() >= INFO) \
        { \
...
#define RLOG_WARN(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= WARN) \
        { \
            ring_log::ins()->try_append("[WARN]", fmt "\n", \
                    ##args); \
        } \
    } while (0)


#define SERVICE_LOG_WARN(fmt, args...) \
    do \
    { \
        if (ring_log::ins(2)->get_level() >= WARN) \
        { \
            ring_log::ins(2)->try_append("[WARN]", fmt "\n", \
                    ##args); \
        } \
    } while (0)

#define RLOG_ERROR(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= ERROR) \
        { \
            ring_log::ins()->try_append("[ERROR]", fmt "\n", \
                    ##args); \
        } \
    } while (0)


#define SERVICE_LOG_ERROR(fmt, args...) \
    do \
    { \
        if (ring_log::ins(2)->get_level() >= ERROR) \
        { \
            ring_log::ins(2)->try_append("[ERROR]", fmt "\n", \
                    ##args); \
        } \
    } while (0)

#define RLOG_FATAL(fmt, args...) \
    do \
    { \
            ring_log::ins()->try_append("[FATAL]", fmt "\n", \
                    ##args); \
    } while (0)
